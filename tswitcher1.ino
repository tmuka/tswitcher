...
int globalPresetMode = 0; //switch between independent and preset global settings
...
int checkForRepeatPress(int thisPin){
    if(thisPin == lastButtonPressed){
        ...
        if(timeDelta > 300 && timeDelta < 1500){ //40-200BPM
...
int checkForChording(int pin_number){
    ...
    return -1; //use negative for false since button zero is not false.
}
...
int detectLongPress(int pin_number){
    if(buttonLastFall[pin_number] > buttonLastRise[pin_number]){ //button is still down
        if((sinceInit - buttonLastFall[pin_number])>3000){ //button has been down over 3 seconds
            ...
            buttonLastRise[pin_number] = sinceInit+5; //reset counter to zero so we dont have multiple saves. add 5 to be sure rise is after fall
            return true;
        }
    }
    return false;
